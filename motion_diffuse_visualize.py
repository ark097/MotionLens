# Replace visualization script in MotionDiffuse/tools/visualization.py
import os
import torch
import numpy as np
import argparse
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        if args.motion_length != -1:
            ...
            result_path = os.path.join(args.result_path, "final_layer")
            os.makedirs(result_path, exist_ok=True)
            plot_t2m(motion, os.path.join(result_path,"example.gif"), os.path.join(result_path,"motion.npy"), title)
            ...
            for i in range(0,12):
                ...
                # Text to motion inference pipeline
                pred_motions = trainer.generate(caption, m_lens, opt.dim_pose, choose_layer=i)
                
                motion = pred_motions[0].cpu().numpy()
                motion = motion * std + mean
                title = args.text + " #%d" % motion.shape[0]
                result_path = os.path.join(args.result_path, f"layer{i}")
                os.makedirs(result_path, exist_ok=True)
                plot_t2m(motion, os.path.join(result_path,"example.gif"), os.path.join(result_path,"motion.npy"), title)